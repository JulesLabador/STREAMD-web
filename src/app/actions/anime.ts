...
export async function getAnimeBySlug(
  ...
  try {
    if (!slug || typeof slug !== 'string') {
      return { success: false, error: 'Invalid slug', code: 'INVALID_INPUT' };
    }
    ...
    const animeRow = data as AnimeRow & {
      anime_studios: Array<{ studio: { id: string; name: string; slug: string } }>;
...
export async function getAnimeById(
  ...
  try {
    if (!id || typeof id !== 'string') {
      return { success: false, error: 'Invalid ID', code: 'INVALID_INPUT' };
    }
...
export async function getStudios(): Promise<ActionResult<StudioWithCount[]>> {
  try {
    ...
    const studios: StudioWithCount[] = (data || []).map((studio) => ({
      ...
      name: studio.name,
      slug: studio.slug,
      animeCount: (studio.anime_studios as Array<{ count: number }>)?.[0]?.count || 0,
    }));
  ...
  try {
    ...
    if (studioError) {
      if (studioError.code === 'PGRST116') {
        return { success: false, error: 'Studio not found', code: 'NOT_FOUND' };
      }
        ...
        data: {
          ...
          anime: {
            ...
            pagination: {
              ...
              totalPages: Math.ceil((totalCount || 0) / validPageSize),
              hasNextPage: false,
              hasPreviousPage: validPage > 1,
            },
          },
    ...
    for (const row of data || []) {
      ...
      if (existing) {
        ...
        seasonMap.set(key, {
          ...
          slug: createSeasonSlug(row.season as AnimeSeason, row.season_year),
          animeCount: 1,
        });
      }
    }
...
export async function getAnimeBySeason(
  ...
  page: number = 1,
  pageSize: number = 24
): Promise<ActionResult<{ seasonInfo: SeasonInfo; anime: PaginatedResponse<Anime> }>> {
  try {
    const validSeasons: AnimeSeason[] = ['WINTER', 'SPRING', 'SUMMER', 'FALL'];
    if (!validSeasons.includes(season)) {
      return { success: false, error: 'Invalid season', code: 'INVALID_INPUT' };
    }
...
export async function getGenres(): Promise<ActionResult<GenreWithCount[]>> {
  try {
    ...
    const genres: GenreWithCount[] = (data || []).map((genre) => ({
      ...
      name: genre.name,
      slug: genre.slug,
      animeCount: (genre.anime_genres as Array<{ count: number }>)?.[0]?.count || 0,
    }));
...
export async function getGenreBySlug(
  ...
  page: number = 1,
  pageSize: number = 24
): Promise<ActionResult<{ genre: { id: string; name: string; slug: string }; anime: PaginatedResponse<Anime> }>> {
  try {
    if (!slug || typeof slug !== 'string') {
      return { success: false, error: 'Invalid slug', code: 'INVALID_INPUT' };
    }
    ...
    if (genreError) {
      if (genreError.code === 'PGRST116') {
        return { success: false, error: 'Genre not found', code: 'NOT_FOUND' };
      }
...
export async function getAnimeByPlatform(
  ...
  page: number = 1,
  pageSize: number = 24
): Promise<ActionResult<{ platformInfo: PlatformInfo; anime: PaginatedResponse<Anime> }>> {
  try {
    ...
    if (!validPlatforms.includes(platform)) {
      return { success: false, error: 'Invalid platform', code: 'INVALID_INPUT' };
    }
